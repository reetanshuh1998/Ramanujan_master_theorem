...
int main(int argc, const char *argv[])
{
    ...
    if (argc != 1 + 2 + 2*0) {
        ...
        for ( const auto& name : B0_standard_pysecdec::names_of_real_parameters )
            std::cout << " " << name;
        for ( const auto& name : B0_standard_pysecdec::names_of_complex_parameters )
            std::cout << " re(" << name << ") im(" << name << ")";
        std::cout << std::endl;
        return 1;
    }

    std::vector<B0_standard_pysecdec::real_t> real_parameters; // = { real parameter values ("psq","msq") go here };
    std::vector<B0_standard_pysecdec::complex_t> complex_parameters; // = { complex parameter values () go here };

    // Load parameters from the command line arguments
    for (int i = 1; i < 1 + 2; i++)
        real_parameters.push_back(B0_standard_pysecdec::real_t(std::atof(argv[i])));

    for (int i = 1 + 2; i < 1 + 2 + 2*0; i += 2) {
        B0_standard_pysecdec::real_t re = std::atof(argv[i]);
        B0_standard_pysecdec::real_t im = std::atof(argv[i+1]);
        complex_parameters.push_back(B0_standard_pysecdec::complex_t(re, im));
    }

    // Set up Integrator
    std::cerr << "Setting up integrator" << std::endl;
    //secdecutil::cuba::Vegas<B0_standard_pysecdec::integrand_return_t> integrator;
    secdecutil::integrators::Qmc<
                                    B0_standard_pysecdec::integrand_return_t,
                                    B0_standard_pysecdec::maximal_number_of_integration_variables,
                                    integrators::transforms::Korobov<3>::type,
                                    B0_standard_pysecdec::user_integrand_t
                                > integrator;
    integrator.verbosity = 1;

    // Construct the amplitudes
    std::cerr << "Generating amplitudes (optimising contour if required)" << std::endl;
    std::vector<B0_standard_pysecdec::nested_series_t<B0_standard_pysecdec::sum_t>> unwrapped_amplitudes =
        B0_standard_pysecdec::make_amplitudes(real_parameters, complex_parameters, "B0_standard_pysecdec_data", integrator);

    // Pack amplitudes into handler
    std::cerr << "Packing amplitudes into handler" << std::endl;
    B0_standard_pysecdec::handler_t<B0_standard_pysecdec::amplitudes_t> amplitudes
    (
    ...
    const std::vector<B0_standard_pysecdec::nested_series_t<secdecutil::UncorrelatedDeviation<B0_standard_pysecdec::integrand_return_t>>> result = amplitudes.evaluate();

    // print the result
    for (unsigned int amp_idx = 0; amp_idx < B0_standard_pysecdec::number_of_amplitudes; ++amp_idx)
