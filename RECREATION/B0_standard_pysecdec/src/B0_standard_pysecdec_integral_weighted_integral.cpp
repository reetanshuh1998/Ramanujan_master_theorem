...
#include "B0_standard_pysecdec_integral_weighted_integral.hpp"

#define INTEGRAL_NAME B0_standard_pysecdec
...
namespace B0_standard_pysecdec_integral
{
    ...
    {
        assert(B0_standard_pysecdec::requested_orders.size() == B0_standard_pysecdec_integral::requested_orders.size());
        size_t number_of_regulators = B0_standard_pysecdec::requested_orders.size();
        std::vector<int> orders; orders.reserve( B0_standard_pysecdec::requested_orders.size() );
        for(size_t i = 0; i < number_of_regulators; ++i)
        {
            int order = (
                              B0_standard_pysecdec::requested_orders.at(i) + 1
                            - B0_standard_pysecdec_integral::lowest_coefficient_orders.at(amp_idx).at(i)
                            - B0_standard_pysecdec_integral::lowest_orders.at(i)
                            - B0_standard_pysecdec_integral::lowest_prefactor_orders.at(i)
                        );
    ...
    {
        std::string coeff_filename(lib_path + "/B0_standard_pysecdec_integral_coefficient" + std::to_string(amp_idx) + ".txt");
...
namespace B0_standard_pysecdec
{
    namespace B0_standard_pysecdec_integral
    {
        ...
        struct WithCuda
        {
            using integrand_t = ::B0_standard_pysecdec_integral::integrand_t;

            // Note: we define make_integrands with B0_standard_pysecdec_contour_deformation
            // but call ::sub_integral_name::make_integrands with B0_standard_pysecdec_integral_contour_deformation
            ...
            {
                    return ::B0_standard_pysecdec_integral::make_integrands
                    (
                        real_parameters,
                        complex_parameters
            ...
            {
                using integrand_t = ::B0_standard_pysecdec_integral::cuda_integrand_t;
                ...
                {
                        return ::B0_standard_pysecdec_integral::make_cuda_integrands
                        (
                            real_parameters,
                            complex_parameters
                            #if B0_standard_pysecdec_integral_contour_deformation
        ...
        // and return one with ::B0_standard_pysecdec_integral::(cuda_)integrand_t instead
        #define INSTANTIATE_AMPLITUDE_INTEGRAL_NONE_QMC() \
            ...
            { \
                using amplitude_integrator_t = secdecutil::integrators::Qmc< \
                     ...
                     INTEGRAL_NAME::maximal_number_of_integration_variables, \
                     ::integrators::transforms::None::type, \
                     ::B0_standard_pysecdec_integral::INTEGRAND_TYPE, \
                     secdecutil::integrators::void_template \
                >; \
        ...
        )
        {
            ...
            using integrand_return_t = B0_standard_pysecdec::integrand_return_t;
            ...
            const std::function<sum_t(const integrand_t& integrand)> convert_integrands =
                [ integrator_ptr ] (const integrand_t& integrand) -> sum_t
                {
                    ...
                    integral_ptr->display_name = ::B0_standard_pysecdec_integral::package_name + "_" + integrand.display_name;
        ...
        )
        {
            ...
            amplitude *= ::B0_standard_pysecdec_integral::prefactor(real_parameters,complex_parameters)
                       * ::B0_standard_pysecdec_integral::coefficient(real_parameters,complex_parameters,amp_idx,lib_path);
            return amplitude;
        }
