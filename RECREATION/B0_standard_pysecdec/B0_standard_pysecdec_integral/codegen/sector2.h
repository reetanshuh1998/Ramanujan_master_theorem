...
#procedure defineExpansion
  Global expansion = SecDecInternalsDUMMYIntegrand;
    Id SecDecInternalsDUMMYIntegrand = (( + (( + (1)) * (( + (1))^(-1)))) * ( + (((( + (1))^( + (1))) * (( + ( + (1)))^( + (0) + (-2))) * (( + ( + (1)))^( + (0))) * (( + (1))^( + (1)))) * (SecDecInternalCalI( + (1)*x0, + (0)))) + (((((( + (1)) * (dSecDecInternalCalId1( + (1)*x0, + (0)))) * ( + (1))))) * ( + (1)))*eps));
...
#procedure insertDecomposed
  ...
  Id F(x0?,eps?) =  + ( + (msq)) + ( + (2*msq - psq))*x0 + ( + (msq))*x0^2;
  Id U(x0?,eps?) =  + ( + (1))*x0 + ( + (1));
