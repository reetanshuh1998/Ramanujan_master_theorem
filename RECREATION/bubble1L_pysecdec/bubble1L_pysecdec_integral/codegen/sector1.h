...
#define name "bubble1L_pysecdec_integral"
...
#define realParameters "s,m1sq,m2sq"
...
#define decomposedPolynomialDerivatives "F,U,dFd0,ddFd0d0"
...
#procedure defineExpansion
  Global expansion = SecDecInternalsDUMMYIntegrand;
    Id SecDecInternalsDUMMYIntegrand = (( + (( + (1)) * (( + (1))^(-1)))) * ( + (((( + (1)*x0^-2)^( + (1))) * (( + ( + (1))*x0^-1)^( + (0) + (-1))) * (( + ( + (1))*x0^-2)^( + (0) + (-1/2))) * (( + (1))^( + (1)))) * (SecDecInternalCalI( + (1)*x0, + (0))))));
...
#procedure insertCalI
    Id SecDecInternalCalI(x0?,eps?) = (SecDecInternalCondefJac( + (1)*x0)) * (SecDecInternalCondefFac( + (1)*x0, + (1)*eps)) * ((U(SecDecInternalDeformedx0( + (1)*x0), + (1)*eps)) ^ ( + (-1) + (2)*eps)) * ((F(SecDecInternalDeformedx0( + (1)*x0), + (1)*eps)) ^ ( + (-1/2) + (-1)*eps));

#endProcedure

#procedure insertOther
    Id SecDecInternalRemainder(x0?,eps?) =  + (1);
  Id SecDecInternalCondefFac(x0?,eps?) = ((SecDecInternalCondefFacx0( + (1)*x0)) ^ ( + (0)));
  Id SecDecInternalOtherPoly0(x0?,eps?) =  + ( + (1));

#endProcedure

#procedure insertDecomposed
    Id F(x0?,eps?) =  + ( + (m2sq))*x0^2 + ( + (m1sq + m2sq - s))*x0 + ( + (m1sq));
  Id U(x0?,eps?) =  + ( + (1)) + ( + (1))*x0;
  Id dFd0(x0?,eps?) =  + (m1sq + m2sq - s) + (2*m2sq)*x0;
  Id ddFd0d0(x0?,eps?) =  + (2*m2sq);
