...
#procedure defineExpansion
  Global expansion = SecDecInternalsDUMMYIntegrand;
    Id SecDecInternalsDUMMYIntegrand = (( + (( + (1)) * (( + (1))^(-1)))) * ( + (((( + (1))^( + (1))) * (( + ( + (1)))^( + (0) + (-1))) * (( + ( + (1)))^( + (0) + (-1/2))) * (( + (1))^( + (1)))) * (SecDecInternalCalI( + (1)*x0, + (0))))));
...
#procedure insertDecomposed
    Id F(x0?,eps?) =  + ( + (m2sq)) + ( + (m1sq + m2sq - s))*x0 + ( + (m1sq))*x0^2;
  Id U(x0?,eps?) =  + ( + (1))*x0 + ( + (1));
  Id dFd0(x0?,eps?) =  + (m1sq + m2sq - s) + (2*m1sq)*x0;
  Id ddFd0d0(x0?,eps?) =  + (2*m1sq);
