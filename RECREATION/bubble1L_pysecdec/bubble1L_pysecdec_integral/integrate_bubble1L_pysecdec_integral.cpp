...
int main(int argc, const char *argv[])
{
    ...
    const std::vector<bubble1L_pysecdec_integral::nested_series_t<bubble1L_pysecdec_integral::integrand_t>> sector_integrands =
        bubble1L_pysecdec_integral::make_integrands(real_parameters, complex_parameters);

    // Add integrands of sectors (together flag)
    std::cerr << "Summing integrands" << std::endl;
    const bubble1L_pysecdec_integral::nested_series_t<bubble1L_pysecdec_integral::integrand_t> all_sectors =
        std::accumulate(++sector_integrands.begin(), sector_integrands.end(), *sector_integrands.begin());

    // Integrate
    std::cerr << "Integrating" << std::endl;
    secdecutil::cuba::Vegas<bubble1L_pysecdec_integral::integrand_return_t> integrator;
