...
extern "C" void
triangle1L_massive_pysecdec_integral__sector_1_order_0__maxdeformp(
...
extern "C" int
triangle1L_massive_pysecdec_integral__sector_1_order_0__fpolycheck(
    const uint64_t lattice,
    const uint64_t index1,
    const uint64_t index2,
    const uint64_t * restrict genvec,
...
)
{
    ...
    for (; index < index2; index += 4) {
        ...
        auto fpoly_im = SecDecInternalImagPart(__Deformedx1Call*tmp1_2 + __Deformedx0Call*tmp1_2 + __Deformedx0Call*__Deformedx1Call*tmp3_4 + msq + msq*SecDecInternalSqr(__Deformedx1Call)+msq*SecDecInternalSqr(__Deformedx0Call));
