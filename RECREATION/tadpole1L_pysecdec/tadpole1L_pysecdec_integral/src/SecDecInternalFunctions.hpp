#ifndef tadpole1L_pysecdec_integral_src_SecDecInternalFunctions_hpp_included
#define tadpole1L_pysecdec_integral_src_SecDecInternalFunctions_hpp_included
    ...
    #define tadpole1L_pysecdec_integral_enforce_complex_return_type 0
        ...
        inline real_t log(real_t arg)
        {
            if (arg < 0)
            {
                    ...
                    error_message += "Encountered \"log(<negative real>)\" in a real-valued integrand function of \"tadpole1L_pysecdec_integral\". ";
    ...
    complex_t inline pow(complex_t x, int y)
    {
        ...
        inline real_t pow(real_t base, real_t exponent)
        {
            if (base < 0)
            {
                    ...
                    error_message += "\"tadpole1L_pysecdec_integral\". Try to enforce complex return values for the generated integrands; i.e. set ";
                    error_message += "\"enforce_complex=True\" in the corresponding call to \"loop_package\" or \"make_package\".";
                    throw std::domain_error(error_message);
                #endif
            }
