...
namespace tadpole1L_pysecdec_integral
{
    ...
    {
        if ( real_parameters.size() != tadpole1L_pysecdec_integral::number_of_real_parameters )
            throw std::logic_error(
                                        "Called \"tadpole1L_pysecdec_integral::make_integrands\" with " +
                                        std::to_string(real_parameters.size()) + " \"real_parameters\" (" +
                                        std::to_string(tadpole1L_pysecdec_integral::number_of_real_parameters) + " expected)."
                                  );

        if ( complex_parameters.size() != tadpole1L_pysecdec_integral::number_of_complex_parameters )
            throw std::logic_error(
                                        "Called \"tadpole1L_pysecdec_integral::make_integrands\" with " +
                                        std::to_string(complex_parameters.size()) + " \"complex_parameters\" (" +
                                        std::to_string(tadpole1L_pysecdec_integral::number_of_complex_parameters) + " expected)."
                                  );
    };
    ...
    )
    {
        check_parameter_sizes(real_parameters, complex_parameters);
            ...
            std::vector<nested_series_t<
                ...
                maximal_number_of_integration_variables,number_of_real_parameters,number_of_complex_parameters,'t','a','d','p','o','l','e','1','L','_','p','y','s','e','c','d','e','c','_','i','n','t','e','g','r','a','l'>
        ...
        #else
            std::vector<nested_series_t<
            secdecutil::CudaIntegrandContainerWithoutDeformation<real_t,complex_t,integrand_return_t,1/*maximal_number_of_functions*/,number_of_real_parameters,number_of_complex_parameters,'t','a','d','p','o','l','e','1','L','_','p','y','s','e','c','d','e','c','_','i','n','t','e','g','r','a','l'>
            >> make_cuda_integrands
            (
                const std::vector<real_t>& real_parameters,
                const std::vector<complex_t>& complex_parameters
