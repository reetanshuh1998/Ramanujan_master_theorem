...
int main(int argc, const char *argv[])
{
    ...
    if (argc != 1 + 1 + 2*0) {
        ...
        for ( const auto& name : tadpole1L_pysecdec::names_of_real_parameters )
            std::cout << " " << name;
        for ( const auto& name : tadpole1L_pysecdec::names_of_complex_parameters )
            std::cout << " re(" << name << ") im(" << name << ")";
        std::cout << std::endl;
        return 1;
    }

    std::vector<tadpole1L_pysecdec::real_t> real_parameters; // = { real parameter values ("msq") go here };
    std::vector<tadpole1L_pysecdec::complex_t> complex_parameters; // = { complex parameter values () go here };

    // Load parameters from the command line arguments
    for (int i = 1; i < 1 + 1; i++)
        real_parameters.push_back(tadpole1L_pysecdec::real_t(std::atof(argv[i])));

    for (int i = 1 + 1; i < 1 + 1 + 2*0; i += 2) {
        tadpole1L_pysecdec::real_t re = std::atof(argv[i]);
        tadpole1L_pysecdec::real_t im = std::atof(argv[i+1]);
        complex_parameters.push_back(tadpole1L_pysecdec::complex_t(re, im));
    }

    // Set up Integrator
    std::cerr << "Setting up integrator" << std::endl;
    //secdecutil::cuba::Vegas<tadpole1L_pysecdec::integrand_return_t> integrator;
    secdecutil::integrators::Qmc<
                                    tadpole1L_pysecdec::integrand_return_t,
                                    tadpole1L_pysecdec::maximal_number_of_integration_variables,
                                    integrators::transforms::Korobov<3>::type,
                                    tadpole1L_pysecdec::user_integrand_t
                                > integrator;
    integrator.verbosity = 1;

    // Construct the amplitudes
    std::cerr << "Generating amplitudes (optimising contour if required)" << std::endl;
    std::vector<tadpole1L_pysecdec::nested_series_t<tadpole1L_pysecdec::sum_t>> unwrapped_amplitudes =
        tadpole1L_pysecdec::make_amplitudes(real_parameters, complex_parameters, "tadpole1L_pysecdec_data", integrator);

    // Pack amplitudes into handler
    std::cerr << "Packing amplitudes into handler" << std::endl;
    tadpole1L_pysecdec::handler_t<tadpole1L_pysecdec::amplitudes_t> amplitudes
    (
    ...
    const std::vector<tadpole1L_pysecdec::nested_series_t<secdecutil::UncorrelatedDeviation<tadpole1L_pysecdec::integrand_return_t>>> result = amplitudes.evaluate();

    // print the result
    for (unsigned int amp_idx = 0; amp_idx < tadpole1L_pysecdec::number_of_amplitudes; ++amp_idx)
