...
int main(int argc, const char *argv[])
{
    ...
    if (argc != 1 + 1 + 2*1) {
        ...
        for ( const auto& name : elliptic2L_pysecdec::names_of_real_parameters )
            std::cout << " " << name;
        for ( const auto& name : elliptic2L_pysecdec::names_of_complex_parameters )
            std::cout << " re(" << name << ") im(" << name << ")";
        std::cout << std::endl;
        return 1;
    }

    std::vector<elliptic2L_pysecdec::real_t> real_parameters; // = { real parameter values ("s") go here };
    std::vector<elliptic2L_pysecdec::complex_t> complex_parameters; // = { complex parameter values ("msq") go here };

    // Load parameters from the command line arguments
    for (int i = 1; i < 1 + 1; i++)
        real_parameters.push_back(elliptic2L_pysecdec::real_t(std::atof(argv[i])));

    for (int i = 1 + 1; i < 1 + 1 + 2*1; i += 2) {
        elliptic2L_pysecdec::real_t re = std::atof(argv[i]);
        elliptic2L_pysecdec::real_t im = std::atof(argv[i+1]);
        complex_parameters.push_back(elliptic2L_pysecdec::complex_t(re, im));
    }

    // Set up Integrator
    std::cerr << "Setting up integrator" << std::endl;
    //secdecutil::cuba::Vegas<elliptic2L_pysecdec::integrand_return_t> integrator;
    secdecutil::integrators::Qmc<
                                    elliptic2L_pysecdec::integrand_return_t,
                                    elliptic2L_pysecdec::maximal_number_of_integration_variables,
                                    integrators::transforms::Korobov<3>::type,
                                    elliptic2L_pysecdec::user_integrand_t
                                > integrator;
    integrator.verbosity = 1;

    // Construct the amplitudes
    std::cerr << "Generating amplitudes (optimising contour if required)" << std::endl;
    std::vector<elliptic2L_pysecdec::nested_series_t<elliptic2L_pysecdec::sum_t>> unwrapped_amplitudes =
        elliptic2L_pysecdec::make_amplitudes(real_parameters, complex_parameters, "elliptic2L_pysecdec_data", integrator);

    // Pack amplitudes into handler
    std::cerr << "Packing amplitudes into handler" << std::endl;
    elliptic2L_pysecdec::handler_t<elliptic2L_pysecdec::amplitudes_t> amplitudes
    (
    ...
    const std::vector<elliptic2L_pysecdec::nested_series_t<secdecutil::UncorrelatedDeviation<elliptic2L_pysecdec::integrand_return_t>>> result = amplitudes.evaluate();

    // print the result
    for (unsigned int amp_idx = 0; amp_idx < elliptic2L_pysecdec::number_of_amplitudes; ++amp_idx)
