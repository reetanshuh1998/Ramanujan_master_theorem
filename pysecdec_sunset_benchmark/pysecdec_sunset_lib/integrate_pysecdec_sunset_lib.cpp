...
int main(int argc, const char *argv[])
{
    ...
    if (argc != 1 + 4 + 2*0) {
        ...
        for ( const auto& name : pysecdec_sunset_lib::names_of_real_parameters )
            std::cout << " " << name;
        for ( const auto& name : pysecdec_sunset_lib::names_of_complex_parameters )
            std::cout << " re(" << name << ") im(" << name << ")";
        std::cout << std::endl;
        return 1;
    }

    std::vector<pysecdec_sunset_lib::real_t> real_parameters; // = { real parameter values ("m1","m2","m3","psq") go here };
    std::vector<pysecdec_sunset_lib::complex_t> complex_parameters; // = { complex parameter values () go here };

    // Load parameters from the command line arguments
    for (int i = 1; i < 1 + 4; i++)
        real_parameters.push_back(pysecdec_sunset_lib::real_t(std::atof(argv[i])));

    for (int i = 1 + 4; i < 1 + 4 + 2*0; i += 2) {
        pysecdec_sunset_lib::real_t re = std::atof(argv[i]);
        pysecdec_sunset_lib::real_t im = std::atof(argv[i+1]);
        complex_parameters.push_back(pysecdec_sunset_lib::complex_t(re, im));
    }

    // Set up Integrator
    std::cerr << "Setting up integrator" << std::endl;
    //secdecutil::cuba::Vegas<pysecdec_sunset_lib::integrand_return_t> integrator;
    secdecutil::integrators::Qmc<
                                    pysecdec_sunset_lib::integrand_return_t,
                                    pysecdec_sunset_lib::maximal_number_of_integration_variables,
                                    integrators::transforms::Korobov<3>::type,
                                    pysecdec_sunset_lib::user_integrand_t
                                > integrator;
    integrator.verbosity = 1;

    // Construct the amplitudes
    std::cerr << "Generating amplitudes (optimising contour if required)" << std::endl;
    std::vector<pysecdec_sunset_lib::nested_series_t<pysecdec_sunset_lib::sum_t>> unwrapped_amplitudes =
        pysecdec_sunset_lib::make_amplitudes(real_parameters, complex_parameters, "pysecdec_sunset_lib_data", integrator);

    // Pack amplitudes into handler
    std::cerr << "Packing amplitudes into handler" << std::endl;
    pysecdec_sunset_lib::handler_t<pysecdec_sunset_lib::amplitudes_t> amplitudes
    (
    ...
    const std::vector<pysecdec_sunset_lib::nested_series_t<secdecutil::UncorrelatedDeviation<pysecdec_sunset_lib::integrand_return_t>>> result = amplitudes.evaluate();

    // print the result
    for (unsigned int amp_idx = 0; amp_idx < pysecdec_sunset_lib::number_of_amplitudes; ++amp_idx)
