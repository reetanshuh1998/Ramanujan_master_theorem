...
extern "C" int
pysecdec_sunset_lib_integral__sector_3_order_n1(
...
extern "C" void
pysecdec_sunset_lib_integral__sector_3_order_n1__maxdeformp(
...
)
{
    ...
    for (; index < index2; index += 4) {
        ...
        x1 = korobov3x3_f(x1);
        auto tmp1_1 = 1-x1;
        auto tmp3_1 = x1*tmp1_1*SecDecInternalSqr(m1);
...
extern "C" int
pysecdec_sunset_lib_integral__sector_3_order_n1__fpolycheck(
    const uint64_t lattice,
    const uint64_t index1,
    const uint64_t index2,
    const uint64_t * restrict genvec,
