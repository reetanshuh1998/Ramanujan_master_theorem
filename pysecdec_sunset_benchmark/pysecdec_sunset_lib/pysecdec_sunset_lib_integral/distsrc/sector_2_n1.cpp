...
extern "C" int
pysecdec_sunset_lib_integral__sector_2_order_n1(
...
extern "C" void
pysecdec_sunset_lib_integral__sector_2_order_n1__maxdeformp(
...
)
{
    ...
    for (; index < index2; index += 4) {
        ...
        x0 = korobov3x3_f(x0);
        auto tmp1_1 = 1-x0;
        auto tmp3_1 = x0*tmp1_1*SecDecInternalSqr(m2);
...
extern "C" int
pysecdec_sunset_lib_integral__sector_2_order_n1__fpolycheck(
    const uint64_t lattice,
    const uint64_t index1,
    const uint64_t index2,
    const uint64_t * restrict genvec,
