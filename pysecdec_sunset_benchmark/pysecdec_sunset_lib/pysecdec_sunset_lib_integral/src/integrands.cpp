...
nested_series_t<sector_container_t> get_integrand_of_sector_6();
    ...
    {
        if (!sectors)
            sectors.reset( new std::vector<nested_series_t<sector_container_t>>{get_integrand_of_sector_1(),get_integrand_of_sector_2(),get_integrand_of_sector_3(),get_integrand_of_sector_4(),get_integrand_of_sector_5(),get_integrand_of_sector_6()} );
        return *sectors;
    };

    void check_parameter_sizes(const std::vector<real_t>& real_parameters, const std::vector<complex_t>& complex_parameters)
    {
        if ( real_parameters.size() != pysecdec_sunset_lib_integral::number_of_real_parameters )
            throw std::logic_error(
                                        "Called \"pysecdec_sunset_lib_integral::make_integrands\" with " +
                                        std::to_string(real_parameters.size()) + " \"real_parameters\" (" +
                                        std::to_string(pysecdec_sunset_lib_integral::number_of_real_parameters) + " expected)."
                                  );

        if ( complex_parameters.size() != pysecdec_sunset_lib_integral::number_of_complex_parameters )
            throw std::logic_error(
                                        "Called \"pysecdec_sunset_lib_integral::make_integrands\" with " +
                                        std::to_string(complex_parameters.size()) + " \"complex_parameters\" (" +
                                        std::to_string(pysecdec_sunset_lib_integral::number_of_complex_parameters) + " expected)."
                                  );
    };
    ...
    )
    {
        check_parameter_sizes(real_parameters, complex_parameters);
            ...
            std::vector<nested_series_t<
                ...
                maximal_number_of_integration_variables,number_of_real_parameters,number_of_complex_parameters,'p','y','s','e','c','d','e','c','_','s','u','n','s','e','t','_','l','i','b','_','i','n','t','e','g','r','a','l'>
        ...
        #else
            std::vector<nested_series_t<
            secdecutil::CudaIntegrandContainerWithoutDeformation<real_t,complex_t,integrand_return_t,1/*maximal_number_of_functions*/,number_of_real_parameters,number_of_complex_parameters,'p','y','s','e','c','d','e','c','_','s','u','n','s','e','t','_','l','i','b','_','i','n','t','e','g','r','a','l'>
            >> make_cuda_integrands
            (
                const std::vector<real_t>& real_parameters,
                const std::vector<complex_t>& complex_parameters
