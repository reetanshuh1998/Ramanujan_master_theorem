...
namespace pysecdec_sunset_lib
{
    ...
    )
    {
        // Construct Integrals
        #if pysecdec_sunset_lib_contour_deformation
            std::vector<nested_series_t<sum_t>> integral_pysecdec_sunset_lib_integral = pysecdec_sunset_lib_integral::make_integral(real_parameters,complex_parameters,integrator,number_of_presamples,deformation_parameters_maximum,deformation_parameters_minimum,deformation_parameters_decrease_factor);
        #else
            std::vector<nested_series_t<sum_t>> integral_pysecdec_sunset_lib_integral = pysecdec_sunset_lib_integral::make_integral(real_parameters,complex_parameters,integrator);
        #endif
        ...
        for (unsigned int amp_idx = 0; amp_idx < number_of_amplitudes; ++amp_idx)
        {
            ...
            amplitude  = pysecdec_sunset_lib_integral::make_weighted_integral(real_parameters, complex_parameters, integral_pysecdec_sunset_lib_integral, amp_idx, lib_path);
