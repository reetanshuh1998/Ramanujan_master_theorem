...
#include "pysecdec_sunset_lib.hpp"
#include "pysecdec_sunset_lib_integral/pysecdec_sunset_lib_integral.hpp"
#include "pysecdec_sunset_lib_integral_weighted_integral.hpp"

#define INTEGRAL_NAME pysecdec_sunset_lib
...
namespace pysecdec_sunset_lib_integral
{
    ...
    {
        assert(pysecdec_sunset_lib::requested_orders.size() == pysecdec_sunset_lib_integral::requested_orders.size());
        size_t number_of_regulators = pysecdec_sunset_lib::requested_orders.size();
        std::vector<int> orders; orders.reserve( pysecdec_sunset_lib::requested_orders.size() );
        for(size_t i = 0; i < number_of_regulators; ++i)
        {
            int order = (
                              pysecdec_sunset_lib::requested_orders.at(i) + 1
                            - pysecdec_sunset_lib_integral::lowest_coefficient_orders.at(amp_idx).at(i)
                            - pysecdec_sunset_lib_integral::lowest_orders.at(i)
                            - pysecdec_sunset_lib_integral::lowest_prefactor_orders.at(i)
                        );
    ...
    {
        std::string coeff_filename(lib_path + "/pysecdec_sunset_lib_integral_coefficient" + std::to_string(amp_idx) + ".txt");
...
namespace pysecdec_sunset_lib
{
    namespace pysecdec_sunset_lib_integral
    {
        ...
        struct WithCuda
        {
            using integrand_t = ::pysecdec_sunset_lib_integral::integrand_t;

            // Note: we define make_integrands with pysecdec_sunset_lib_contour_deformation
            // but call ::sub_integral_name::make_integrands with pysecdec_sunset_lib_integral_contour_deformation
            ...
            (
                ...
                #if pysecdec_sunset_lib_contour_deformation
            ...
            {
                    return ::pysecdec_sunset_lib_integral::make_integrands
                    (
                        real_parameters,
                        complex_parameters
            ...
            {
                using integrand_t = ::pysecdec_sunset_lib_integral::cuda_integrand_t;
                ...
                {
                        return ::pysecdec_sunset_lib_integral::make_cuda_integrands
                        (
                            real_parameters,
                            complex_parameters
                            #if pysecdec_sunset_lib_integral_contour_deformation
        ...
        // and return one with ::pysecdec_sunset_lib_integral::(cuda_)integrand_t instead
        #define INSTANTIATE_AMPLITUDE_INTEGRAL_NONE_QMC() \
            ...
            { \
                using amplitude_integrator_t = secdecutil::integrators::Qmc< \
                     ...
                     INTEGRAL_NAME::maximal_number_of_integration_variables, \
                     ::integrators::transforms::None::type, \
                     ::pysecdec_sunset_lib_integral::INTEGRAND_TYPE, \
                     secdecutil::integrators::void_template \
                >; \
        ...
        )
        {
            ...
            using integrand_return_t = pysecdec_sunset_lib::integrand_return_t;
            ...
            const std::function<sum_t(const integrand_t& integrand)> convert_integrands =
                [ integrator_ptr ] (const integrand_t& integrand) -> sum_t
                {
                    ...
                    integral_ptr->display_name = ::pysecdec_sunset_lib_integral::package_name + "_" + integrand.display_name;
        ...
        )
        {
            ...
            amplitude *= ::pysecdec_sunset_lib_integral::prefactor(real_parameters,complex_parameters)
                       * ::pysecdec_sunset_lib_integral::coefficient(real_parameters,complex_parameters,amp_idx,lib_path);
            return amplitude;
        }
