(*
 ...
 #
 #  PURPOSE
 #  takes output from sector decomposition, manipulates it using 
 #  various symmetries of the problem to form 'integrandfunctionlist', 
 #  and also exstore,ustore,fstore,nstore,degen, which are then used as the
 #  input for symbsub.m and form*C.m. Length of 'integrandfunctionlist' 
 #  gives the number of different exponent structures. 
 #    
 #  INPUTS
 #  
 ...
 #
 #  RESULT
 #  integrandfunctionlist is formed, as are exstore, ustore, fstore, nstore and degen, for use by formfortran.m and symbsub.m  
 #  
 #  SEE ALSO
...
If[
   dieflag==0
   ,
   (* reorder the exponents in exponent list: 
      highest exponents first, lowest last, rename z[i]
      such that z[1] is first again *)
   ...
   populatefudegn[popsec_]:=Block[{},
				  fudegn++;
				  (*store exponents expoF and expoU*)
                                  exstore[iflcount,fudegn]=popsec[[5]]; 
   ...
   collatelist[cplist_]:=Block[{integrandfunctionlist},
			       ...
			       (* split integrandfunctionlist every time the exponentlist 
				and expoU, expoF are no longer the same *)
			       tempintegrandfunctionlist=Split[integrandfunctionlist, 
							       (#1[[1]] == #2[[1]] && #1[[5]] == #2[[5]]) &]; 
			       iflcount=0;
   ...
   Get[StringJoin[srcdir,"Degeneracy.m"]];	
   onlyz[expr_]:=Block[{a,b,c,d},Return[(expr/.{Times[z[a_],z[b_]]->0,Power[z[c_],d_]->0})]];
   
   (********************
    Those functions which are symmetric to another don`t need to be calculated twice, hence, degen 
    is set =0 and this particular function is not used (computed or written) in formContour(P)C.m 
    ************************)
   ...
   If[
      pbytes<20000000
      ,  
      degentime=AbsoluteTiming[findsymmetries;][[1]];
