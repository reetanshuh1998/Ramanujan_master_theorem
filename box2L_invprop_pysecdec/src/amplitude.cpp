...
namespace box2L_invprop_pysecdec
{
    ...
    )
    {
        // Construct Integrals
        #if box2L_invprop_pysecdec_contour_deformation
            std::vector<nested_series_t<sum_t>> integral_box2L_invprop_pysecdec_integral = box2L_invprop_pysecdec_integral::make_integral(real_parameters,complex_parameters,integrator,number_of_presamples,deformation_parameters_maximum,deformation_parameters_minimum,deformation_parameters_decrease_factor);
        #else
            std::vector<nested_series_t<sum_t>> integral_box2L_invprop_pysecdec_integral = box2L_invprop_pysecdec_integral::make_integral(real_parameters,complex_parameters,integrator);
        #endif
        ...
        for (unsigned int amp_idx = 0; amp_idx < number_of_amplitudes; ++amp_idx)
        {
            ...
            amplitude  = box2L_invprop_pysecdec_integral::make_weighted_integral(real_parameters, complex_parameters, integral_box2L_invprop_pysecdec_integral, amp_idx, lib_path);
