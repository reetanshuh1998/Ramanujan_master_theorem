...
#include "box2L_invprop_pysecdec_integral_weighted_integral.hpp"

#define INTEGRAL_NAME box2L_invprop_pysecdec
...
namespace box2L_invprop_pysecdec_integral
{
    ...
    {
        assert(box2L_invprop_pysecdec::requested_orders.size() == box2L_invprop_pysecdec_integral::requested_orders.size());
        size_t number_of_regulators = box2L_invprop_pysecdec::requested_orders.size();
        std::vector<int> orders; orders.reserve( box2L_invprop_pysecdec::requested_orders.size() );
        for(size_t i = 0; i < number_of_regulators; ++i)
        {
            int order = (
                              box2L_invprop_pysecdec::requested_orders.at(i) + 1
                            - box2L_invprop_pysecdec_integral::lowest_coefficient_orders.at(amp_idx).at(i)
                            - box2L_invprop_pysecdec_integral::lowest_orders.at(i)
                            - box2L_invprop_pysecdec_integral::lowest_prefactor_orders.at(i)
                        );
    ...
    {
        std::string coeff_filename(lib_path + "/box2L_invprop_pysecdec_integral_coefficient" + std::to_string(amp_idx) + ".txt");
...
namespace box2L_invprop_pysecdec
{
    namespace box2L_invprop_pysecdec_integral
    {
        ...
        struct WithCuda
        {
            using integrand_t = ::box2L_invprop_pysecdec_integral::integrand_t;

            // Note: we define make_integrands with box2L_invprop_pysecdec_contour_deformation
            // but call ::sub_integral_name::make_integrands with box2L_invprop_pysecdec_integral_contour_deformation
            ...
            {
                    return ::box2L_invprop_pysecdec_integral::make_integrands
                    (
                        real_parameters,
                        complex_parameters
            ...
            {
                using integrand_t = ::box2L_invprop_pysecdec_integral::cuda_integrand_t;
                ...
                {
                        return ::box2L_invprop_pysecdec_integral::make_cuda_integrands
                        (
                            real_parameters,
                            complex_parameters
                            #if box2L_invprop_pysecdec_integral_contour_deformation
        ...
        // and return one with ::box2L_invprop_pysecdec_integral::(cuda_)integrand_t instead
        #define INSTANTIATE_AMPLITUDE_INTEGRAL_NONE_QMC() \
            ...
            { \
                using amplitude_integrator_t = secdecutil::integrators::Qmc< \
                     ...
                     INTEGRAL_NAME::maximal_number_of_integration_variables, \
                     ::integrators::transforms::None::type, \
                     ::box2L_invprop_pysecdec_integral::INTEGRAND_TYPE, \
                     secdecutil::integrators::void_template \
                >; \
        ...
        )
        {
            ...
            using integrand_return_t = box2L_invprop_pysecdec::integrand_return_t;
            ...
            const std::function<sum_t(const integrand_t& integrand)> convert_integrands =
                [ integrator_ptr ] (const integrand_t& integrand) -> sum_t
                {
                    ...
                    integral_ptr->display_name = ::box2L_invprop_pysecdec_integral::package_name + "_" + integrand.display_name;
        ...
        )
        {
            ...
            amplitude *= ::box2L_invprop_pysecdec_integral::prefactor(real_parameters,complex_parameters)
                       * ::box2L_invprop_pysecdec_integral::coefficient(real_parameters,complex_parameters,amp_idx,lib_path);
            return amplitude;
        }
