...
int main(int argc, const char *argv[])
{
    ...
    const std::vector<pysecdec_C0_unequal_lib_integral::nested_series_t<pysecdec_C0_unequal_lib_integral::cuda_integrand_t>> sector_integrands =
        pysecdec_C0_unequal_lib_integral::make_cuda_integrands(real_parameters, complex_parameters);

    // Add integrands of sectors (together flag)
    std::cerr << "Summing integrands" << std::endl;
    const pysecdec_C0_unequal_lib_integral::nested_series_t<pysecdec_C0_unequal_lib_integral::cuda_together_integrand_t> all_sectors =
        std::accumulate(++sector_integrands.begin(), sector_integrands.end(), pysecdec_C0_unequal_lib_integral::cuda_together_integrand_t()+*sector_integrands.begin());

    // Integrate
    std::cerr << "Integrating" << std::endl;
    secdecutil::integrators::Qmc<
                                    pysecdec_C0_unequal_lib_integral::integrand_return_t,
                                    pysecdec_C0_unequal_lib_integral::maximal_number_of_integration_variables,
                                    integrators::transforms::Korobov<3>::type,
                                    pysecdec_C0_unequal_lib_integral::cuda_together_integrand_t
                                > integrator;
