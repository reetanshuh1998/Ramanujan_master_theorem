...
primarysec[np_,Finp_,Uinp_,Num_,xl_]:=
  Block[{fF,fU,nfF,nfU,eF,eU,pF,pU,
    ...
    (* debug *) (*Print["pNum=",pNum];Print["Numnew=",Numnew];*)
    minpF=Simplify[pF/. {t[i_]->0}];
    minpU=Simplify[pU/. {t[i_]->0}]; 
    (* minpNum=Simplify[pNum/. {t[i_]->0}]; eliminated 18.12.14; not needed; leads to 1/0 if
    numerator contains t[i]^(-1-eps) from eps propagator powers *) 
	    ...
	    If[MatchQ[fx[j],1]==False,
		  ...
		  If[samepow==True
	       ...
	       While[factored==False,
		     ...
		     If[MatchQ[(Nfac/.ignoreeps)/.fx[j]->0,0]==False,factored=True]];(*Print["Nfac=",Nfac];*)
	       posreps2={a_ fx[j]^(b_ + c_ eps)->{b+c eps},a_ fx[j]^(c_ eps)->{c eps},a_ fx[j]^(b_+ eps)->{b+eps},a_ fx[j]^(eps)->{eps},
			 fx[j]^(b_ + c_ eps)->{b+c eps},fx[j]^(c_ eps)->{c eps},fx[j]^(b_+ eps)->{b+eps},fx[j]^(eps)->{eps},a_ fx[j]^(b_)->{b},fx[j]^(b_)->{b},fx[j]->{1},a_ fx[j]->{1}};
	       defac=Dfac/.posreps2;
	       (*Print["defac1=",defac];*)
	       defac=If[ListQ[defac],defac[[1]],False];
	       (*Print["defac2=",defac];*)
