...
BeginPackage["Format`"]
FORMATREPS={};(*########## Added for SecDec, July 23rd 2011 ###########*)
...
]& @ (Experimental`OptimizationSymbol /. {opts} /.
...
AssignCase->Default, AssignEnd->";", AssignFortranNumbers->False,
AssignHyperbolic->False,
...
(* Quiet[] added JC 10/11/11*)
CAssign[lhs_:"",expr_?(!OptionQ[#]&),opts___?OptionQ]:=
  Module[{optvals},
    optvals /; 
      And[
        (optvals =
OptionTest[expr,GetShape[lhs],CAssign,System`Utilities`FilterOptions[CAssign,opts]])=!=$Failed,
        optvals =
CMain[lhs,expr,optvals,{System`Utilities`FilterOptions[Experimental`OptimizeExpression,opts]}];
        True
      ]
  ];
  ...
  Module[{optvals},
    optvals /;
      And[
        ...
        optvals =
FMain[lhs,expr,optvals,{System`Utilities`FilterOptions[Experimental`OptimizeExpression,opts]}];
        True
      ]
  ];
...
Literal[MRH[List[x__]]]:=
SequenceForm[OutputForm["["],MapleArgs[",",x],OutputForm["]"]];
...
(**** Maple function head redefinitions. ****)
    ...
    MD[Abs]=abs; MD[AiryAi]=Ai; MD[AiryBi]=Bi; MD[And]=and; MD[ArcCos]=arccos;
MD[ArcCosh]=arccosh;
    ...
    MD[Erfc]=erfc; MD[EulerE]=euler; MD[Exp]=exp; MD[Expand]=expand;
MD[ExpIntegral]=Ei;
    ...
    MD[Min]=min; MD[Mod]=mod; MD[N]=evalf; MD[NIntegrate]=int; MD[NSolve]=fsolve;
MD[Not]=not;
...
MapleFun[f_,x__]:=
SequenceForm[MD[f],OutputForm["("],MapleArgs[",",x],OutputForm[")"]];
...
(* Apply string replacement rules and indentation/termination strings. *)
    ...
    strings =
      Map[
        ...
        StringReplace[ Flatten[{strings}],Join[arep,FORMATREPS] ]
      ];
						(*######### Added for SecDec July 23rd 2011 ############*)
...
FindPositions[expr_,0]:=
  Position[
Function[x,UnsameQ[x,0]&&UnsameQ[x,Null],Listable][expr],True,Heads->False];
...
CheckOptimization[_Experimental`OptimizeExpression, expr_]:=
(Message[AssignOptimize::fail]; expr);
...
CheckRange[expr_,form_,{xrmin_,xrmax_,xihmin_,xihmax_,ximin_,ximax_,prec_}]:=
  Module[{complxQ,intmsg=True,intQ,realQ,rlmsg=True},
    ...
    intQ = i_Integer?(#>ximax||#<ximin&):>
             (If[intmsg,  intmsg=False;
Message[AssignRange::integer,xihmin,xihmax,prec]];
                N[i] /. realQ);
